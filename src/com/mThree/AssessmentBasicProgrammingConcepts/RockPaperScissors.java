...
public class RockPaperScissors {
	/*
	 * we're building a Rock paper scissors app that takes a user input so we need
	 * account for user error loops etc simple so rock>scissors>paper>rock
	 * 
	 * additionally user gives number of rounds with max of 10 and min of 1 so the
	 * assignment wants to me numerically represent the signs. (not what i'd do, but
	 * okay) comp does random and chooses a sigil and we go from there 3 out comes:
	 * tie, greater, lesser need to track wins and losses after rounds then we
	 * declare the winner and do flavoer text whole thing is in a loop? so it can do
	 * it again :/ that sucks cause its better to do this as another class that
	 * calls this function--or another method ^^ so we have main method that returns
	 * and loops the game a method that tracks wins and losses a method that retuns
	 * winner/loser a method for validation
	 * 
	 * 
	...
	public static void main(String[] args) {
		...
		int playerChooice, numberOfRounds = 0, numberOfWins = 0, numberOfLosses = 0, numberOfTies = 0,
				validationAttempts = 0, comptuterRoll;
		...
		/*
		 * so the whole thing needs to be in a try,catch then proceed to a loop IF they
		 * choosen correctly
		 */
		...
		while (playerDecisionToPlay) {
			...
			/*
			 * so player decided to play now we need them to choose number of rounds since
			 * specifically I'm to terminate if they enter something out of min/max then
			 * this isn't a loop
			 */
			try {
			...
			} catch (Exception e) {// to befair only exception that could be thrown is null or some how crashing
									// scanner
				System.out.println(
						"You dare mock me with entering something weird!?\n" + "How dare you!\nI say good to you!");
				System.exit(0);
			}

			/*
			 * now we have rounds we do a loop to call the function of rock paper scissors
			 * so hours of banging my head and reconfigering code. couldn't figure out
			 * problems with switch statements... its cause i forgot in java u need to put
			 * break; in each case :C i feel defeated not total loss i cleaned up the
			 * validation heavily
			 */
			...
			for (int x = 1; x <= numberOfRounds; x++) {
				...
				while (validationAttempts < MAX_ATTEMPTS_VALIDATION) {
					...
					default:
						if (validInput == false) {
							System.out.println(playerInputDecision + "? " + "Let's try that again...");
							validationAttempts++;
							break;
						}
					}
				...
				if (outputNumbers[0] == 1) {
					System.out.println("\nIt's a Tie!");
				}
				if (outputNumbers[1] == 1) {
					System.out.println("\nLooks like you lost this round.");
				}
				if (outputNumbers[2] == 1) {
					System.out.println("\nCongrats, you won this round.");
				}
				System.out.println(
						"\t\tWins: " + numberOfWins + "\t\tLosses: " + numberOfLosses + "\tTies: " + numberOfTies);

			}
			if (numberOfLosses == numberOfWins) {
				System.out.println("GG! Looks like it's a draw.\nPlay again?");
			}
			if (numberOfLosses < numberOfWins) {
				System.out.println("Well played! You beat me!\nPlay again?");
			}
			if (numberOfLosses > numberOfWins) {
	...
	/*
	 * might be able to over load this update: overloaded and overrided -- maybe not
	 * cause examples have them in another sub class ... i don't remember class and
	 * sub level class stuff. to be fair I THINK theres a way to condense into 1
	 * method for reuse i'm not very skilled in java --> lets think this over s
	 */
	public static Boolean inputValidation(String inputResponse) {
		Boolean output = null;
		if (inputResponse == null || inputResponse.isEmpty()) {
			output = false;
		} else if (inputResponse.equalsIgnoreCase("yes") || inputResponse.equalsIgnoreCase("y")
				|| inputResponse.equalsIgnoreCase("1")) {
			output = true;
		} else if (inputResponse.equalsIgnoreCase("no") || inputResponse.equalsIgnoreCase("n")
	...
	public static Boolean inputValidation(String inputResponse, Boolean sentinal) {
		Boolean output = null;
		if ((inputResponse.equalsIgnoreCase("rock") || inputResponse.equalsIgnoreCase("paper")
				|| inputResponse.equalsIgnoreCase("Scissors") || inputResponse.equalsIgnoreCase("1")
				|| inputResponse.equalsIgnoreCase("2") || inputResponse.equalsIgnoreCase("3")
				|| inputResponse.equalsIgnoreCase("r") || inputResponse.equalsIgnoreCase("p")
	...
	public static int whoWon(int playerRoll, int computerRoll) {
		/*
		 * permutations are r:r s:s p:p which are all just == (tie) r<p s<r p<s which
		 * are all comp wins r>s s>p p>r which are all player wins
		 */
		int outputOfWinner;
		if ((playerRoll - 1) == computerRoll) {
			outputOfWinner = 0;
		} else if (((playerRoll - 1) == 0 && computerRoll == 1)// player rolled rock comp rolled paper
	...
	public static int whoWon(String playerRoll, int computerRoll) {
		...
		if (((playerStringComparator.equalsIgnoreCase("r")) && computerRoll == 0)// r'r
				|| ((playerStringComparator.equalsIgnoreCase("p")) && computerRoll == 1)// p'p
				|| ((playerStringComparator.equalsIgnoreCase("s")) && computerRoll == 2)) {
			outputOfWinner = 0;
		} else if (((playerStringComparator.equalsIgnoreCase("r")) && computerRoll == 1)// player rolled rock comp
																						// rolled paper
				|| ((playerStringComparator.equalsIgnoreCase("p")) && computerRoll == 2)// player rolled paper comp
																						// rolled scissors
				|| ((playerStringComparator.equalsIgnoreCase("s")) && computerRoll == 0))// player rolled scissors and
																							// comp rolled rock
		{
			outputOfWinner = 1;
		} else {
	...
	/*
	 * at this juncture what is left to TODO? welp we can stylize, cause gui >no gui
	 * we can consolidate and clean up code (originally i was planning to make a
	 * validation class and exception, but that's bad coding for java apparently)
	 * i'm not sure... i was gunna say a method to do the validation retries or
	 * inputs... but is there a point to doing that? theres also tons of varriables
	 * eating memory , could consolidate and resuse
	 * 
	 */
}
